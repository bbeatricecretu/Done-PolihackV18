...
class AIAgentService {
  ...
  async processNotificationsWithAgent(notifications) {
    ...
    try {
      ...
      // Fetch context notifications and existing tasks for each source
      ...
      const prompt = `You have ${notifications.length} new notifications to process. Analyze each one and determine if it should create, edit, or delete a task.
...
For each notification:
1. Check "recentContext" - if there are previous notifications from the same source/author, this might be an update/cancellation
2. Decide the action:
   - CREATE: New task (use create_task_from_notification)
   - EDIT: Modifies existing task from same author (use edit_task - you'll need to find the task by searching recent tasks from this source)
   - DELETE: Cancels previous task (use delete_task)
   - IGNORE: Not task-worthy (spam, social media, etc.)
3. Call the appropriate MCP tool
4. Always call delete_notification() for each notification after processing

Examples:
- "Buy milk" → CREATE task
- Later: "Don't buy milk, buy eggs instead" (with context showing "buy milk") → EDIT previous task
- Later: "Nevermind, don't buy anything" → DELETE the task

Process all notifications now.`;
...
To find the task to edit/delete:] API Error:', response.status, errorText);
...
Your workflow:
1. Analyze each notification and check "recentContext" for previous notifications from the same source/author
2. Determine the action needed:
   - CREATE: New task (no related previous notifications)
   - EDIT: Update existing task (notification modifies/changes previous request from same author)
   - DELETE: Cancel task (notification cancels previous request)
   - IGNORE: Not task-worthy
3. Call the appropriate MCP tool (create_task_from_notification, edit_task, delete_task, or mark_task_complete)
4. Always call delete_notification() after processing each notification
...
To find the task to edit/delete:
1. Use get_tasks_by_filter() with the source_app to find recent tasks from that app
2. Match the task based on content similarity or creation time
3. Then call edit_task() or delete_task() with the correct task ID

NOTIFICATION ANALYSIS GUIDELINES:s?.[0]?.message?.tool_calls || [],
        success: true
      };
   ...
   */
  getSystemPrompt() {
    return `You are an intelligent task management assistant that processes Android notifications and decides whether they should become tasks.

Your workflow:
1. Analyze each notification to determine if it's relevant for task creation
2. If relevant, extract task information and call create_task_from_notification() via MCP
3. Always call delete_notification() via MCP after processing each notification (whether you created a task or not)
...
- **Prefer EDIT over CREATE when in doubt about similarity**`;m the same source:
1. Search for the related task using get_tasks_by_filter(source_app: <app>)
2. Find the most recent task that matches the context
3. Apply the appropriate action (edit, delete, or complete)

IMPORTANT:
- Process notifications efficiently and check context first
- Always clean up by calling delete_notification() for each processed notification
- Use context to avoid creating duplicate tasks when someone is modifying a request
- Keep titles concise and actionable
- Be consistent with category and priority assignments
...
- Always clean up by calling delete_notification() for each processed notification
- If uncertain about relevance, err on the side of NOT creating a task
- Keep titles concise and actionable
- Be consistent with category and priority assignments`;
  }
