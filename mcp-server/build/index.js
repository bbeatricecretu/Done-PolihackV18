...
async function handleSkipNotification(args) {
    if (!cosmosClient || !process.env.COSMOS_DATABASE || !process.env.COSMOS_CONTAINER) {
        return {
            content: [
                {
...
async function handleCreateTaskFromChat(args) {
    const request = pool.request();
    const id = generateUUID();
    request.input('id', sql.UniqueIdentifier, id);
    request.input('title', sql.NVarChar(500), args.title);
...
async function handleSuggestTasksByContext(args) {
            ...
            try {
                ...
                if (data.results && data.results.length > 0) {
                    suggestions.push({
                        ...
                        nearby_locations: data.results.slice(0, 3).map((loc) => ({
...
async function handleGetImportantTasks(args) {
...
async function handleGetTasksForLocation(args) {
...
async function handleGenerateSearchQuery(args) {
