...
import { syncTasksToCloud, fetchLocationsFromCloud } from './src/services/CloudSync';
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const syncLocations = async () => {
      try {
        const cloudLocations = await fetchLocationsFromCloud();
        setSavedLocations(cloudLocations);
      } catch (error) {
        console.error('[App] Failed to sync locations:', error);
      }
    };
    ...
    const locationsInterval = setInterval(syncLocations, 20000); // 20s
